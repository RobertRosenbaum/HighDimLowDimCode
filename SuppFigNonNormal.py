...
x[0,int(t1on/dt):int(t1off/dt),:] = cx*(u+(W1@v-v)/c)
...
c0='e'
ax0 = axes[c0]
xvmax=torch.max(torch.cat((torch.abs(xv),torch.abs(xurand)))).item()
zvmax=torch.max(torch.cat((torch.abs(zv),torch.abs(zurand)))).item()
xzvmax=max(xvmax,zvmax)

I2plot=int(2000/dt)
numplot=N
ax0.plot(ToNP(zv[:I2plot]),ToNP(zurand[:I2plot]),color=zclr,lw=1)
ax0.axis([-xzvmax,xzvmax,-xzvmax,xzvmax])
ax0.set_xticks([-2,0,2])
ax0.set_yticks([-2,0,2])
ax0.set_xticklabels([])
ax0.set_yticklabels([])
ax0.set_xlabel(r'$\mathbf{v}\cdot\mathbf{z}$')
ax0.set_ylabel(r'$\mathbf{u}_{rand}\cdot\mathbf{z}$')
ax0.set_title(c0,loc='left')

c0='f'
ax0 = axes[c0]
ax0.plot(torch.real(lamW),torch.imag(lamW),'.',markersize=4, color=Wclr)
ax0.set_xlabel(r'real$(\lambda_W)$')
ax0.set_ylabel(r'imag$(\lambda_W)$')
ax0.set_title(c0,loc='left')


# c0='f'
# ax0 = axes[c0]
# ax0.plot(ToNP(xv[:I2plot]),ToNP(xurand[:I2plot]),color=xclr,lw=1)
# ax0.axis([-xzvmax,xzvmax,-xzvmax,xzvmax])
# ax0.set_xticks([-2,0,2])
# ax0.set_yticks([-2,0,2])
# ax0.set_xticklabels([])
# ax0.set_yticklabels([])
# ax0.set_xlabel(r'$\mathbf{v}\cdot\mathbf{x}$')
...
ax0.plot(time2[I1plot:I2plot],ToNP(xv[I1plot:I2plot]),label=r'$\mathbf{v}\cdot\mathbf{x}$', color=xclr)
ax0.plot(time2[I1plot:I2plot],ToNP(yv[I1plot:I2plot]),label=r'$\mathbf{v}\cdot W\mathbf{z}$', color=yclr)
ax0.plot(time2[I1plot:I2plot],ToNP(zv[I1plot:I2plot]),label=r'$\mathbf{v}\cdot\mathbf{z}$', color=zclr)
...
if SaveFigs:
    fig.savefig('./Figures/SuppFigNonNormal.pdf')
    fig.savefig('./Figures/SuppFigNonNormalunpolished.svg')
