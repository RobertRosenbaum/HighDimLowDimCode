...
import torch 
from utils import ToNP, MakeSmoothGaussianProcess, TorchPCA, DrawFFwdNet
...
def FFNN(x,W,eta):
    ...
    z=torch.zeros(batch_size,Nt,N)
    for i in range(1,Nt):
        z[:,i,:]=z[:,i-1,:]+eta*(-z[:,i-1,:]+f(x[:,i,:]@W))
    return z
...
if SaveFigs:
    fig.savefig('./Figures/SuppFigFfwd.pdf')
    fig.savefig('./Figures/SuppFigFfwdunpolished.svg')
