...
### Now simulate with step-based input to test response to random
### vs aligned inputs
cx=10
...
# Duration of sim
T1 = t2off+tbreak
time1=np.arange(0,T1+dt,dt)
Nt1=len(time1)
t_span = [0, T1]

# Build input
x1 = np.zeros((Nt1, N))
x1[int(t1on/dt):int(t1off/dt),:] = cx*v
x1[int(t2on/dt):int(t2off/dt),:] = cx*urand
...
sol = solve_ivp(qmf_sis_pert, t_span, z_fp, integration_method,
...
fig, axes = plt.subplot_mosaic("a;b",figsize=(4,4.5))
