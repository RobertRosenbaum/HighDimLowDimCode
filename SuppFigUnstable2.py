...
# Rescale c so that it has the same ratio with rho as in Figure 1. 
# This makes W similarly low rank in each case.
# If we kept c the same, then it wouldn't be much larger than
# sigma.
c=10
rho=0.5
...
# Strength of stimuli
# This needs to be larger to be visible on top of chaos
# In stable case, the scale of cx doesn't matter because system is 
# linear and stable. Here, it does.
cx=np.sqrt(N) 
...
ax0.set_title(c0,loc='left',x=0.1,y=1.05)
...
# Manual positioning
axes['a'].set_position([.01, 0.7, 0.17, 0.21]) 

if SaveFigs:
    fig.savefig('./Figures/SuppFigUnstable2.pdf')
    fig.savefig('./Figures/SuppFigUnstable2unpolished.svg')
